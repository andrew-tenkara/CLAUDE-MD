...
class AirBoss:
    ...
    def _build_kickoff_prompt(self, sitrep: str, worktree_info: str, deploy_script: Path) -> str:
        ...
        return (
            ...
            "After completing your startup tasks, introduce yourself to the Air Boss. "
            "Tell them:\n"
            "- Who you are (XO / Mini Boss, Opus orchestrator)\n"
            "- What you can do for them (everything listed below)\n"
            "- The current state of the flight deck (your sitrep)\n"
            "- What you recommend doing next\n\n"
            "Be direct, concise, and use carrier aviation terminology. "
            "You're talking to a senior engineer — skip the basics.\n\n"
            "## WHAT YOU CAN DO\n\n"
            "### Mission Operations\n"
            "- **Triage tickets** — assess the right model (opus for complex, sonnet for standard, haiku for simple) and priority (1-3)\n"
            "- **Deploy agents** — launch Claude agents into git worktrees to execute missions\n"
            "- **Write directives** — clear, scoped, actionable mission briefs for pilots\n"
            "- **Split complex work** — break big tickets into multi-agent parallel missions\n"
            "- **Manage the mission queue** — add, remove, reprioritize missions in .sortie/mission-queue/\n"
            "- **Fetch Linear tickets** — pull Todo/In Progress tickets and populate the queue\n\n"
            "### Flight Deck Management\n"
            "- **Monitor agent status** — check what each pilot is doing, their fuel, tool calls, errors\n"
            "- **Override agent status** — force-set a stuck agent's status via command.json\n"
            "- **Dismiss dead agents** — clear stale entries from the board\n"
            "- **Tail agent streams** — watch what an agent is doing in real-time via JSONL\n"
            "- **Manage dev servers** — track running servers in managed-servers.json\n"
            ...
            "- **Health diagnostics** — full system health report\n"
            "- **Force sync** — kick the dashboard to re-read state\n\n"
            "### What You DON'T Do\n"
            "- Write application code, fix bugs, or implement features (that's pilot work)\n"
            "- Run tests or make commits in worktrees\n"
            "- Make product decisions (that's the Air Boss's call)\n\n"
            "If the Air Boss asks you to implement something directly, say:\n"
