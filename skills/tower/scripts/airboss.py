...
class AirBoss:
    ...
    def _build_kickoff_prompt(self, sitrep: str, worktree_info: str, deploy_script: Path, db_health: str = "") -> str:
        ...
        return (
            ...
            "\n"
            "DIRECTIVE CONTENT — what to put in --directive:\n"
            "The directive is the full mission briefing the pilot reads before writing a single line of code.\n"
            "A good directive answers: What's broken/needed? Why? Where in the code? What does 'done' look like? What branch do I PR against?\n"
            "\n"
            "Before deploying, ALWAYS build a rich directive. Steps:\n"
            "  1. Call mcp__linear__get_issue with the ticket ID (includeRelations: true)\n"
            "  2. Extract: title, description, acceptance criteria, labels, priority, comments\n"
            "  3. Determine the BASE BRANCH:\n"
            "     - Check what branch the ticket's parent feature lives on (e.g. feat/operations-sourcing-tab)\n"
            "     - If the ticket is part of an active feature branch, the pilot PRs against THAT branch, not dev\n"
            "     - If standalone, the pilot PRs against dev\n"
            "     - ALWAYS state this explicitly — pilots default to dev if you don't tell them\n"
            "  4. Check storage DB for prior intel on this ticket:\n"
            f"       python3 '{Path(__file__).parent / 'storage-db.py'}' get-briefing '{ctx._project_dir}' <TICKET-ID>\n"
            "  5. Identify key files — grep/glob for the area of code the pilot will touch.\n"
            "     Give them 2-5 specific file paths to start from. Don't make them search blind.\n"
            "  6. Add XO intel: related tickets, known gotchas, architecture constraints, what NOT to do\n"
            "  7. If the Air Boss gave you context (screenshots, root cause, clarification), include it\n"
            "     under '### Air Boss Context' — this is the highest-signal section of any directive\n"
            "\n"
            "Directive template:\n"
            "  ## Ticket: <ID> — <title>\n"
            "  **Priority**: <priority> | **Labels**: <labels> | **Parent**: <parent-id if any>\n"
            "  **Base branch**: <branch to PR against> (e.g. feat/operations-sourcing-tab or dev)\n"
            "  **Status**: <current status> (note if work was previously started)\n"
            "  \n"
            "  ### Description\n"
            "  <full ticket description — verbatim from Linear>\n"
            "  \n"
            "  ### Air Boss Context\n"
            "  <any context the Air Boss provided: root cause analysis, screenshots description,\n"
            "   clarification of what the real problem is vs what the ticket says, UX intent, etc.\n"
            "   This section is GOLD — it's the stuff not in Linear that makes the difference\n"
            "   between shipping the right fix vs the literal fix.\n"
            "   OMIT this section entirely if the Air Boss provided no additional context.>\n"
            "  \n"
            "  ### The Fix\n"
            "  <concrete, specific instructions for what to change. Not 'fix the bug' — describe\n"
            "   the approach: 'Disable the checkbox on already-searched rows by adding a disabled\n"
            "   prop when material.status === searched'. The more specific, the fewer round-trips.>\n"
            "  \n"
            "  ### Key Files\n"
            "  <2-5 specific file paths the pilot should start from. Include line numbers if known.>\n"
            "  - src/frontend/views/operations/leads-scanner/search-tool.tsx — materials table\n"
            "  - src/shared/types/leads-scanner.ts — SupplierSearchResult type (lines 246-300)\n"
            "  \n"
            "  ### Acceptance Criteria\n"
            "  <checklist format — from Linear if present, or synthesize from description>\n"
            "  - [ ] Concrete, testable criterion\n"
            "  - [ ] Another criterion\n"
            "  \n"
            "  ### Scope Boundaries\n"
            "  <what the pilot should NOT do — prevents scope creep>\n"
            "  - Do NOT refactor the search API — this is frontend only\n"
            "  - Do NOT touch the green highlighting logic — it already works\n"
            "  \n"
            "  ### XO Intel\n"
            "  <related tickets, prior debrief from storage DB, known gotchas, architecture notes>\n"
            "  <worktree status — does one already exist? was work started previously?>\n"
            "\n"
            "QUALITY BAR:\n"
            "A pilot reading this directive should be able to start coding within 2 minutes.\n"
            "If they'd need to ask 'what branch do I PR against?' or 'where is this code?' or\n"
            "'what does done look like?' — the directive is incomplete. Fix it before deploying.\n"
            "\n"
            "The storage-db briefing is also auto-appended by deploy-agent.sh — do not duplicate it, "
            "but DO add anything you know that the DB doesn't have yet.\n"
            "\n"
            "Examples:\n"
            f"  bash '{deploy_script}' ENG-200 --model sonnet "
            f"--branch 'eng/eng-200-auth-token-rotation' "
            f"--directive '## Ticket: ENG-200 — Auth token rotation\n"
            f"**Priority**: urgent | **Labels**: Bug | **Parent**: ENG-198\n"
            f"**Base branch**: feat/operations-sourcing-tab\n"
            f"**Status**: Todo\n\n"
            f"### Description\nImplement sliding-window refresh token rotation. Tokens expire after 7 days...\n\n"
            f"### The Fix\nAdd rotation logic in src/server/services/auth/token-service.ts. On each token use,\n"
            f"issue a new refresh token and invalidate the old one in Redis.\n\n"
            f"### Key Files\n- src/server/services/auth/token-service.ts — main rotation logic\n"
            f"- src/server/repositories/session-repository.ts — Redis session store\n\n"
            f"### Acceptance Criteria\n- [ ] Tokens rotate on use\n- [ ] Old token invalidated immediately\n\n"
            f"### Scope Boundaries\n- Do NOT change the JWT signing algorithm\n- Do NOT modify the login flow\n\n"
            f"### XO Intel\nRelated: ENG-195 (JWT middleware rewrite, merged). Watch out for the Redis session store — see gotcha in storage DB.' "
            f"--project-dir '{ctx._project_dir}'\n"
            ...
            '  "model": "sonnet",\n'
            '  "agent_count": 1,\n'
            '  "directive": "## Ticket: ENG-200 — Auth token rotation\\n**Priority**: urgent\\n\\n### Description\\n<full ticket description>\\n\\n### Acceptance Criteria\\n<criteria>\\n\\n### XO Intel\\n<prior intel, gotchas>",\n'
            ...
            "The TUI scans .claude/worktrees/ every 5 seconds. For a pilot to appear on the sortie list:\n"
            "  1. Worktree directory must exist under .claude/worktrees/<TICKET-ID>/\n"
            "  2. .sortie/directive.md must exist inside the worktree (only hard requirement)\n"
            "  3. The ticket must NOT be in the dismissed set (in-memory, resets on Tower restart)\n\n"
            "DISMISS BEHAVIOR (Z key or xo-tools dismiss):\n"
            "  - Removes the pilot from the board\n"
            "  - Adds ticket ID to _dismissed_tickets (in-memory set — prevents resurrection)\n"
            "  - Deletes the worktree (git worktree remove --force, fallback to rm -rf)\n"
            "  - Compresses ticket debriefs into a summary\n"
            "  - If the worktree is later RECREATED (e.g. via /tq), the dismiss is automatically\n"
            "    cleared on the next sync cycle — the pilot reappears on the board.\n\n"
            "TROUBLESHOOTING — PILOT NOT VISIBLE ON BOARD:\n"
            "  1. Was it dismissed earlier this session? → Recreate the worktree (/tq <ticket>)\n"
            "     and the un-dismiss logic will pick it up on the next 5s sync.\n"
            "  2. Is .sortie/directive.md present? → ls <worktree>/.sortie/directive.md\n"
            "  3. Does read_sortie_state see it? → Run the xo-tools board command.\n"
            "     If it appears in the xo-tools output but not the TUI, restart Tower.\n"
            "  4. Nuclear option: restart Tower (clears all in-memory state including dismissals).\n\n"
            "WORKTREE OPS:\n"
