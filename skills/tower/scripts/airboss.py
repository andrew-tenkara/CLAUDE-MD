...
class AirBoss:
    ...
    def _build_kickoff_prompt(self, sitrep: str, worktree_info: str, deploy_script: Path, db_health: str = "") -> str:
        ...
        return (
            ...
            "Before deploying, ALWAYS fetch the full Linear ticket and build a rich directive. Steps:\n"
            "  1. Call mcp__linear__get_issue with the ticket ID\n"
            "  2. Extract: title, description, acceptance criteria, labels, priority, comments\n"
            "  3. Check storage DB for prior intel on this ticket:\n"
            f"       python3 '{Path(__file__).parent / 'storage-db.py'}' get-briefing '{ctx._project_dir}' <TICKET-ID>\n"
            "  4. Add any cross-cutting context YOU know as XO (related tickets, gotchas, architecture notes)\n"
            "  5. Combine into a single directive string — ticket context first, your XO intel second\n"
            "\n"
            "Directive template:\n"
            "  ## Ticket: <ID> — <title>\n"
            ...
            "  \n"
            "  ### XO Intel\n"
            "  <prior debrief summary, related tickets, known gotchas, architecture notes>\n"
            "\n"
