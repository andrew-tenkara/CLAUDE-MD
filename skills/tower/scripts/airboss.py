...
class AirBoss:
    ...
    def _build_kickoff_prompt(self, sitrep: str, worktree_info: str, deploy_script: Path) -> str:
        ...
        return (
            ...
            "the environment is ready. Run preflight, then guide the user through setup interactively.\n\n"
            f"STEP 0 — PREFLIGHT:\n"
            ...
            "  Tell the Air Boss exactly what's needed and where to get it:\n"
            "    'I need an Anthropic API key for the sentinel's Haiku gate.\n"
            "     Get one at: https://console.anthropic.com/settings/keys\n"
            f"     Then paste it here and I\\'ll add it to {Path.home() / '.zshrc'} for you.'\n"
            "  When they paste the key, write the export line to their shell rc:\n"
            f"    echo 'export ANTHROPIC_API_KEY=\"<key>\"' >> {Path.home() / '.zshrc'}\n"
            "  Then source it in the current session so it takes effect immediately.\n\n"
