...
6. **Write the plan**: Write \`.sortie/plan.html\` as a fully self-contained HTML doc, modeled on \`${TEMPLATES_DIR}/plan-template.html\`. Mirror the \`<head>\` (styles + Prism + Mermaid CDN), keep the rail / header / classification scaffolding, and replace the \`<article id="doc">\` body with your own content. Use \`superpowers:writing-plans\` discipline (DRY, YAGNI, TDD, bite-sized tasks) but **translate its markdown-flavored examples to HTML as you write**: \`# heading\` → \`<h2>…</h2>\`, \`- item\` → \`<ul><li>…</li></ul>\`, \`\\\`code\\\`\` → \`<code>…</code>\`, fenced \`\\\`\\\`\\\`lang … \\\`\\\`\\\`\` → \`<pre><code class="language-lang">…</code></pre>\`. No markdown in the HTML body — ever. Keep the visible body under ~250 lines — a 1,000-line plan has as many surprises as 1,000 lines of code. The Code Reference Library at the bottom doesn't count.
