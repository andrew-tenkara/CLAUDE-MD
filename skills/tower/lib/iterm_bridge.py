...
class ItermBridge:
    ...
    def open_agent_pane(self, pilot: "Pilot") -> None:
        ...
        kickoff = (
            ...
            f"Invoke superpowers:brainstorming, then superpowers:writing-plans (use its "
            f"DRY/YAGNI/TDD/bite-sized-tasks discipline, but translate its markdown examples "
            f"to HTML as you go: # heading -> <h2>, - item -> <ul><li>, `code` -> <code>, "
            f"```lang … ``` -> <pre><code class='language-lang'>… No markdown in the body). The plan MUST "
